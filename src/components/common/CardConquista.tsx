  ...
  return (
    <div
      className={`relative p-5 rounded-2xl border transition-all duration-300 flex flex-col items-center text-center space-y-3 shadow-sm select-none ${
        bloqueado
          ? 'grayscale opacity-60 bg-gray-100 border-slate-200 text-slate-500'
    ...
    >
      ...
      {bloqueado && (
        <div className="absolute top-3 right-3 text-slate-400 bg-slate-200/50 p-1 rounded-full" title="Bloqueado">
      ...
      <div className="space-y-1">
        <h4 className={`font-heading font-extrabold text-label-md ${bloqueado ? 'text-slate-600' : 'text-on-surface'}`}>
          {titulo}
        </h4>
        <p className={`font-sans text-label-sm ${bloqueado ? 'text-slate-400' : 'text-on-surface-variant'}`}>
          {descricao}
        </p>
      </div>
